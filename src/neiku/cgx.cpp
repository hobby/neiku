...
CCgx::CCgx(): m_pCGI(NULL), m_bInit(false), m_bAccepted(false)
{
...
const char* CCgx::GetErrMsg()
{
    return m_sLastErrMsg.c_str();
}

const char* CCgx::GetValue(const char* szName, const char* szDefVal)
{
    CHECK_INIT(szDefVal);
...
int32_t CCgx::GetValueI(const char* szName, int32_t iDefVal)
{
...
int64_t CCgx::GetValueII(const char* szName, int64_t lDefVal)
{
    const char* szValue = GetValue(szName, NULL);
    if (szValue == NULL)
    {
...
uint32_t CCgx::GetValueUI(const char* szName, uint32_t dwDefVal)
{
    const char* szValue = GetValue(szName, NULL);
    if (szValue == NULL)
    {
...
uint64_t CCgx::GetValueUII(const char* szName, uint64_t ddwDefVal)
{
    const char* szValue = GetValue(szName, NULL);
    if (szValue == NULL)
    {
...
int CCgx::SetValue(const char* szName, const char* szValue)
{
    CHECK_INIT(-1);
...
int CCgx::SetValue(const char* szName, int iValue)
{
    CHECK_INIT(-1);
...
// FILE* CCgx::GetFile(const char* szName)
// {
//     CHECK_INIT(NULL);
...
void CCgx::Redirect302Url(const char* fmt, ...)
{
    CHECK_INIT();
...
// 302 to /path/to/xxx
void CCgx::Redirect302Uri(const char* fmt, ...)
{
    CHECK_INIT();
...
int CCgx::Render(const char* szTplPath)
{
    CHECK_INIT(-1);
...
int CCgx::Render(const char* szTplPath, STRING& stOutput)
{
    CHECK_INIT(-1);
...
void CCgx::SetErrMsg(NEOERR* pError)
{
    if(pError == STATUS_OK)
    {
        m_sLastErrMsg = "";
...
void CCgx::SetErrMsg(const char* szErrMsg)
{
    if(szErrMsg != NULL)
    {
